...
source "qemu" "gosjava11" {
    ...
    accelerator = "kvm"
    http_content = {
        "/preseed.cfg" = templatefile("${path.root}/../debian/trixie.pkrtpl", {tasks="ssh-server", pkg="", boot="/dev/vda"})
    }
...
source "virtualbox-iso" "gosjava11" {
    iso_url = var.iso_url
    iso_checksum = var.iso_checksum
    disk_size = 30000
    memory = 5120
    ...
    http_content = {
        "/preseed.cfg" = templatefile("${path.root}/../debian/trixie.pkrtpl", {tasks="ssh-server", pkg="", boot="/dev/sda"})
    }
    ssh_username = "vagrant"
    ssh_password = "password"
    ssh_timeout = "40m"
...
build {
    sources = ["source.qemu.gosjava11", "source.virtualbox-iso.gosjava11"]
    provisioner "shell" {
        scripts = [
